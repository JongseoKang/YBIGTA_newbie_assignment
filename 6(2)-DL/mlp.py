...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h1 = X @ W1 + b1  # Linear transformation for layer 1
        z1 = relu(h1)     # Apply activation function (e.g., ReLU)

        h2 = z1 @ W2 + b2  # Linear transformation for layer 2
        z2 = relu(h2)      # Apply activation function

        h3 = z2 @ W3 + b3  # Linear transformation for layer 3
        z3 = relu(h3)      # Apply activation function

        h4 = z3 @ W4 + b4  # Output layer transformation
        y_hat = sigmoid(h4)  # Apply sigmoid for binary classification
        y_hat = y_hat.flatten()  # Ensure y_hat has shape (N,)
